...
describe('noteToMarkdown', () => {
  it('exports note content, metadata, and related notes', () => {
    ...
    expect(markdown).toContain('## Auditoria IA')
    expect(markdown).toContain('- Proveedor IA: qwen')
    expect(markdown).toContain('- Modelo IA: qwen3\\.5:0\\.8b')
    expect(markdown).toContain('- Analizado: 2026-06-15T00:00:00.000Z')
    expect(markdown).toContain('- Duracion: 840 ms')
  ...
  it('uses stable fallbacks when summary, tags, or links are missing', () => {
    ...
    expect(markdown).toContain('## Auditoria IA')
    expect(markdown).toContain('- Proveedor IA: Sin auditoria')
...
describe('fine-tune dataset export', () => {
  it('builds local supervised JSONL examples from analyzed notes', () => {
    ...
    expect(examples[0].messages[0].content).toContain('Categorias permitidas: Inbox, Trabajo, Proyecto')
    expect(examples[0].messages[0].content).toContain('No ejecutes herramientas ni asumas permisos')
    expect(examples[0].messages[0].content).toContain('No incluyas razonamiento')
    expect(examples[0].messages[1].content).toContain('Fecha de referencia:\n2026-06-15')
    expect(examples[0].messages[1].content).toContain('Metadatos actuales:')
    expect(examples[0].messages[1].content).toContain('Titulo: Roadmap Neuronotes')
    expect(examples[0].messages[1].content).toContain('Categoria actual: Proyecto')
    expect(examples[0].messages[1].content).toContain('Etiquetas actuales: qwen, notas')
